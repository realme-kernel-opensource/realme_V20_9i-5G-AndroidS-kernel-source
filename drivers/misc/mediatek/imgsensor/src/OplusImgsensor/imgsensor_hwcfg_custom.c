        ...
        enum   IMGSENSOR_HW_POWER_STATUS pwr_status)
{
    ...
    pr_info("[%s] sensor_idx:%d pwr_status: %d\n", __func__, sensor_idx, pwr_status);
        ...
        enum   IMGSENSOR_HW_POWER_STATUS pwr_status)
{
    ...
    pr_info("[%s] pmic_ldo_get_type:%d pwr_status:%d sensorIdx:%d, pin:%d[RST:2, AVDD:3, DVDD:4, DOVDD:5, MCLK:14]",
    ...
    if(pin != IMGSENSOR_HW_PIN_AVDD && pin != IMGSENSOR_HW_PIN_AVDD_1
        ...
        pr_info("can't use the fan53870 to power this powertype.");
        return IMGSENSOR_RETURN_ERROR;
    }
    if (pin == IMGSENSOR_HW_PIN_AFVDD && sensor_idx != IMGSENSOR_SENSOR_IDX_MAIN) {
        pr_info("can't use the fan53870 to power afvdd.");
        return IMGSENSOR_RETURN_ERROR;
    }
    if (pwr_status == IMGSENSOR_HW_POWER_STATUS_ON) {
        //Power on the sensor.
